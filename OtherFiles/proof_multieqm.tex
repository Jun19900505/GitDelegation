...
\end{align*}
Denote $a=\frac{\omega_{s}}{\omega_{\eta}}\beta^{2}$ and $h=\frac{\omega_{u}}{\omega_{\eta}}$ 
so that the fixed-point problem is rewritten as 
\begin{equation}
a=W(\alpha)\equiv h\left(1+2\theta\left(\frac{a}{a+1}\right)^{2}\right).\label{eq:W}
\end{equation}
Note that $W'(a)=4h\theta\frac{a}{(a+1)^{3}}$ and $W''(a)=4h\theta\frac{1-a}{(a+1)^{4}}$. Also, $W'(0)=0$ and $\lim_{a\rightarrow\infty}W'(a)=0$. Thus, $W(a)$ is monotonically increasing in $a$, is convex in $a<1$, and is concave in $a>1$. 

To find the solutions, rewrite (\ref{eq:W}) to obtain 
\begin{align}
0 & =G(a)\nonumber \\
 & =a^{3}+(2-2\theta h-h)a^{2}+(1-2h)a-h.\label{eq:G}
\end{align}
As $G(0)=-h<0$ and $\lim_{a\rightarrow\infty}G(a)=\infty$, it has at least one solution. Also,
\[
g(a)\equiv G'(a)=3a^{2}+2(2-2\theta h-h)a+1-2h.
\]
If $2h>1$, then $g(0)<0$, and thus $g(a)=0$ has a unique solution. $G(a)$ takes a U-shaped curve, and therefore, there is a unique solution to $G(a)=0$.

Focus on $2h<1$. Then, the discriminant of $g$ is
\begin{align*}
...
\end{equation}
If either one of them is violated, $g(a)>0$ holds for all $a\ge0$, and $G(a)$ is monotonically increasing in $a$ so that $G(a)=0$ has a unique solution. 

Suppose that conditions (\ref{eq:cond1}) and (\ref{eq:cond2}) are satisfied and denote the two solutions of $g(a)=0$ as $a_{1}$ and $a_{2}(>a_{1})$. The local maximum and minimum of $G$ are, respectively $G(a_{1})$ and $G(a_{2})$. There are three cases: 
\begin{enumerate}
\item If, and only if, $G(a_{1})>0$ and $G(a_{2})<0$, $G(a)=0$ has three solutions. We denote them as $a_{L},a_{M},$ and $a_{H}$ ($a_{L}<a_{M}<a_{H}$);
\item If $G(a_{1})>0$ and $G(a_{2})>0$, then $G(a)=0$ has a unique solution that corresponds to $a_{L}$; and
\item If $G(a_{1})<0$ and $G(a_{2})<0$, then $G(a)=0$ has a unique solution that corresponds to $a_{H}$. 
\end{enumerate}
Applying $g(a_{k})=0$ ($k=1,2$), it holds that 
\begin{align*}
G(a_{k}) & =2a_{k}\left[3(1-2h)-(2-2\theta h-h)^{2}\right]\\
 & -(2-2\theta h-h)(1-2h)-9h.
\end{align*}
Hence, $G(a_{1})>0$ and $G(a_{2})<0$ are equivalent to the first and the second inequalities in the following:
\[
a_{1}<\hat{a}\equiv\frac{\theta h(1-2h)-(1+h)^{2}}{(1+h)^{2}-4\theta h\left(2-h-\theta h\right)}<a_{2}.
\]
These conditions are satisfied if, and only if, $g(\hat{a})<0$. This value is expressed as 
\[
g(\hat{a})=\bar{g}R,
\]
where $\bar{g}>0$ is a function of $\theta$ and $h$ but always positive, and 
\[
R\equiv2h^{3}(1+2\theta)^{2}+2h^{2}(3-10\theta)+h(6-\theta)+2.
\]
Note that $g(\hat{a})<0$ is a stronger condition than (\ref{eq:cond1}) and (\ref{eq:cond2}) because, if $g(\hat{a})<0$ with $\hat{a}>0$, then $g=0$ has two solutions. In summary, $G=0$ has three solutions if, and only if, $R<0$. By rewriting $R$ as a function of $\theta$, we obtain 
\begin{equation}
R(\theta)=8h^{3}\theta^{2}+\theta h(8h^{2}-20h-1)+2(1+h)^{3}.\label{eq:R}
\end{equation}
Taking $R$ as a quadratic function of $\theta$, its discriminant is $D_{R}=-h^{2}(8h-1)^{3}.$ If $8h>1$, then $D_{R}<0$ holds, meaning that $R>0$ for all $h>0$. Hence, $G(a)$ is monotonically increasing in $a$ and the solution is unique. If, in contrast, $8h<1$, then $D_{R}>0$. Also, the second term of $R$ is negative, $8h^{2}-20h-1<0$ (due to $8h<1$). Thus, $R(\theta)=0$ has two solutions, denoted as $\theta_{1}$ and $\theta_{2}$ ($>\theta_{1}$), such that, $R<0$ if, and only if, $\theta\in[\theta_{1},\theta_{2}]$. Hence, if $\theta$ lies strictly between $\theta_{1}$ and $\theta_{2}$, then $G(a)=0$ has three solutions. If $\theta\le\theta_{1}$, then $G(a_{1})\ge0$ and $G(a_{2})>0$, so that case 2 arises, and the solution is unique at $a_{L}$. If $\theta\ge\theta_{H}$, then $G(a_{1})<0$ and $G(a_{2})\le0$, so that case 3 arises, and the solution is unique at $a_{H}$. In summary, we obtain the following lemma:
\begin{lemma}
\label{lem:multiple sol}(i) If $8h\ge1$, then the fixed-point problem (\ref{eq:W}) has a unique (stable) solution.\\
(ii) If $8h<1$, then there exist thresholds of $\theta$ denoted as $\theta_{1}$ and $\theta_{2}$ ($>\theta_{1}$) and obtained as solutions to $R(\theta)=0$ in (\ref{eq:R}). In this case, (ii-a) if $\theta\in(\theta_{1},\theta_{2})$, the fixed-point problem has three solutions, $a_{L},a_{M},$ and $a_{H}$ ($a_{L}<a_{M}<a_{H})$, where $a_{L}$ and $a_{H}$ are stable; (ii-b) if $\theta\le\theta_{1}$, then the problem has a unique solution at $a_{L}$; and (ii-c) if $\theta\ge\theta_{2}$, then the problem has a unique solution at $a_{H}$.
\end{lemma}

\paragraph*{\textmd{\textit{Classification of an unique equilibrium.}}}

As the definition of the high- and the low-$\beta$ equilibria is not trivial in case (i) of Lemma \ref{lem:multiple sol}, we consider the following classification. First, note that the fixed-point problem (\ref{eq:W}) has the following properties:
\begin{lemma}
\label{lem:slope}(i) If $\theta<\tilde{\theta}\equiv\frac{27}{16h}$, then $W$ is less steep than the 45-degree line for all $a\ge0$.\\
(ii) If $\theta\ge\tilde{\theta}$, then there exist $a=\alpha_{1}$ and $\alpha_{2}$ ($>\alpha_{1}$), such that $W'(a)>1\Leftrightarrow a\in(\alpha_{1},\alpha_{2})$. 
\end{lemma}
Accordingly, define $a=\hat{\alpha}$ as the smallest valeu of $a$ such that $W'(a)=1$. Lemma \ref{lem:slope} implies that 
\[
\hat{\alpha}=\begin{cases}
\infty & \text{if }\theta<\tilde{\theta},\\
\alpha_{1} & \text{if }\theta\ge\tilde{\theta}.
\end{cases}
\]
We classify the high- and low-$\beta$ equilibrium (i.e., $a_{L}$ and $a_{H}$) in case (i) of Lemma \ref{lem:multiple sol} according to the following definition.
\begin{definition}
\label{def:def1}When $8h>1$ and the solution to the fixed-point problem (\ref{eq:W}) is uniqe at $a=a^{*}$, we define the solution as $a_{L}$ if $a^{*}<\hat{\alpha}$. Otherwise, it is classified as $a_{H}$.
\end{definition}
Following Definition \ref{def:def1}, $8h>1$ and $\theta<\tilde{\theta}$ always lead to a unique low-$\beta$ equilibrium. In contrast, if $8h>1$ and $\theta\ge\tilde{\theta}$, then we have the following result:
\begin{lemma}
If $8h>1$ and $\theta\ge\tilde{\theta}$, the unique equilibrium of the fixed-point problem (\ref{eq:W}) is always the high-$\beta$ equilibrium.
\end{lemma}
\begin{proof}
Denote the solution to $a=W(a)$ as $a^{*}$. It suffices to show that $\theta\ge\tilde{\theta}\Rightarrow a^{*}>1$, as $a=1$ is the tipping point of the hump-shaped curve $W'(a)$ and $\alpha_{1}<1$. Note that $a^{*}>1$ is equivalent to $W(1)>1$, which in turn reduces to $\theta>\hat{\theta}\equiv\frac{9}{2}(\frac{1}{2h}-1)$. Since $8h>1\Leftrightarrow\tilde{\theta}>\hat{\theta}$, $\theta\ge\tilde{\theta}$ implies $a^{*}>1>\alpha_{1}$.
\end{proof}

\paragraph*{\textmd{\textit{Characterizing the boundaries by $\omega_{\eta}$. }}}

Suppose that $8h<1$. Note that 
\begin{align*}
h\frac{\partial R}{\partial h} & =2\theta h(1+10h)-6(1+h)^{2}.
\end{align*}
Denote 
\[
\bar{\theta}\equiv\frac{3(1+h)^{2}}{h(1+10h)},
\]
so that $\frac{\partial R}{\partial h}\gtrless0\Leftrightarrow\theta\gtrless\bar{\theta}$. At $\theta=\bar{\theta}$, $R(\bar{\theta})<0$  due to $8h<1$. Therefore, $\theta_{1}<\bar{\theta}<\theta_{2}$ hold. Also, 
\[
\frac{\partial R(\theta_{1})}{\partial h}<0,\ \frac{\partial R(\theta_{2})}{\partial h}>0.
\]
 Since $\frac{\partial R(\theta_{1})}{\partial\theta}<0$ and $\frac{\partial R(\theta_{2})}{\partial\theta}<0$, the implicit function theorem implies
\[
\frac{d\theta_{k}}{dh}=-\frac{\partial R(\theta_{k})/\partial h}{\partial R(\theta_{k})/\partial\theta}<0
\]
for both $k=1$ and $2$. Thus, $\theta_{1}$ and $\theta_{2}$ are monotonically increasing in $\omega_{\eta}$. Also, at the limit of $h\rightarrow\frac{1}{8}$, it holds that $\theta_{1}\rightarrow\theta_{2}$. Hence, by the squeeze theorem, $\theta_{1}=\theta_{2}=\frac{3(1+h)^{2}}{h(1+10h)}|_{h=\frac{1}{8}}=\frac{27}{2}$ at this limit. Moreover, $\tilde{\theta}=\frac{27}{16h}$ is increasing in $\omega_{\eta}$ with $\tilde{\theta}=0$ at $\omega_{\eta}=0$ and $\tilde{\theta}=\frac{27}{2}$ at $\omega_{\eta}=8\omega_{u}$. Therefore, we define a unique $\omega_{\eta}$ that satisfies $\theta_{1}(\omega_{\eta})=\theta$ as $\omega_{H}$ and that satisfies $\theta_{2}(\omega_{\eta})=\theta$ as $\omega_{L}$. Finally, $\omega_{0}$ is defined by $\tilde{\theta}(\omega_{\eta})=\theta$. Due to the monotone mapping, $\omega_{0}$, $\omega_{L},$ and $\omega_{H}$ are increasing in $\theta$.
% ========================================================
% By using equations (\ref{lambda}) and (\ref{eq:xi}), function $F$ in (\ref{F}) is represented as
% \begin{align*}
% F(\beta) & =\frac{1}{2\beta\omega_{s}}\frac{(\beta^{2}\omega_{s}+\omega_{u})(\beta^{2}\omega_{s}+\omega_{\eta})^{2}}{(\beta^{2}\omega_{s}+\omega_{\eta})^{2}-\theta\omega_{u}\beta^{2}\omega_{s}}.
