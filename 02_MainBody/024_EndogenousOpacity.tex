...
At the begenning of $t=0$, the fund manager chooses the fund opacity, $\omega_{e}$. Subsequently, but prior to the investment delegation, each investor decides whether to participate in the fund by paying the verification cost $C(\omega_{e})$ with $C(0)=0$ and $C'(\cdot)>0$, which we justify below. The investors' participation decisions collectively determine the size of clientele, $n$. In the end of $t=1$, each client observes the asset's payoff and its price, $(\delta,p)$, and receives the payment from the fund according to the manager's report about fund proceeds, denoted as $\tilde{y}$.

\paragraph{\normalfont{\it{Misreporting and verification.}}}

The fund manager has an incentive to misreport the fund proceeds. Namely, by reporting $\tilde{y}$, she pays out $(1-\phi)\tilde{y}$ to clients and captures $y-(1-\phi)\tilde{y}$ on her own, suggesting that reporting $\tilde{y}=0$ would be optimal.\footnote{Technically, the manager earns an infinite payment by reporting $\tilde{y}=-\infty$, while we assume that clients must receive non-negative payoffs from the fund (i.e., they do not pay additional money), thereby imposing $\tilde{y}\ge0$.} By paying the verification cost, $C(\omega_{e})$, each client observes $x$ in the end of $t=1$ and, together with the market data $(\delta,p)$, she can verify $y=(\delta-p)x+M$, disproving $\tilde{y}\neq y$.\footnote{We implicitly assume that the manager is penalized and incurs a negative large utility if her misreporting is detected.} Otherwise, the manager can misreport $\tilde{y}=0$, as there are levels of $e$ and $u$ that are consistent with the report, i.e., she claims that the investment was $\tilde{x}=-M/(\delta-p)$, while the noise components in the fund information and order flow were $\tilde{e}=f-\tilde{x}$ and $\tilde{u}=(x+u)-\tilde{x}$, making $f$ and $p$ consistent with $\tilde{y}=0$. The verification cost is increasing in the degree of fund opacity, $C'(\omega_{e})>0$, capturing the fact that backing out the information about $x$ becomes harder as the noise variance in the fund information becomes larger.
...
Each investor becomes a fund client and delegates her capital to the fund if the following inequality holds:
\begin{equation}
