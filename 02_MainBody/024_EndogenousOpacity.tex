...
For each level of $\beta$, equation (\ref{eq:mi_max}) indicates that the optimal capital allocation in (\ref{eq:symmetricm}) leads to the following expected payoffs for an investor:
...
\begin{equation}
     ...
     %&\nonumber
\end{equation}
where $\beta = \beta_{H}$ or $\beta_{L}$. The first term in the middle of (\ref{eq:pibeta}) represents the share of the trading profit allocated to the client, and the second term is the cost assosiated with delegation, $\phi m$, in the equilibrium. The last equality arises from the expected trading profit: $\mathrm{E}[\pi] = \frac{\beta \omega_{s}\omega_{u}}{\beta^2 \omega_u + \omega_s}$. Therefore, she is willing to participate in the fund and pays the verification cost if and only if 
\begin{equation}
     \bar{V}_I \equiv \sum_{j=H,L} \rho_{j}V_{I}(\beta_{j}) \geq C(\omega_{e}),\label{eq:participation} 
\end{equation}
where $\bar{V}_{I}$ represents the investor's \textit{ex-ante} expected payoff, incorporating multiple equilibria. 

Conversely, conditional on the skill, the manager expects to earn $\mathrm{E}[\phi y|s] $
As shown below, the size of clientele ($n$) increases as long as inequality (\ref{eq:participation}) holds, leading to the equilibrium $n$ and influencing the manager's compensation. For each level of $\beta$, the manager's expected payoff is
\begin{equation}
...
\end{equation}
Therefore, incorporating the reaction of her clients on fund opacity, the manager solves
\begin{equation}
...
\begin{enumerate}
     ...
     \item There is a unique $n=n(\omega_e)$ that satisfies the break-even condition, and it serves as the equilibrium size of clientele. Moreover, $n(\omega_{e})$ is decreasing function of $\omega_{e}$ as long as the marginal verification cost is sufficiently large.
\end{enumerate}
%===========================================
