...
This section analyzes endogenous opacity by allowing the manager to control $\omega_{e}$. It also endogenizes the negative reaction of the fund investorele to opacity, capturing the cost of making the fund opaque for the manager. We find that she strategically chooses opacity as a commitment device to restrict herself from taking excessive risk.

\subsection{Setup} \label{sec:setupEndog}
We extend the model to a two-stage setting ($t = 0, 1$), assuming that the baseline model analyzed in the previous sections takes place in the second period ($t = 1$). In the first period ($t=0$), the fund manager chooses the fund opacity $\omega_{e}$, and each investor chooses whether to become a fund investor by participating in the fund. We assume that participating in the fund takes a non-pecuniary participation cost, $C$, defined below. The investors' participation decisions collectively determine the size of investorele $n$. In the end of $t=1$, each investor observes the realized asset's payoff $\delta$ and its price $p$, and receives the payment from the fund according to the manager's report about fund proceeds, denoted as $\tilde{y}$.

\paragraph{\normalfont{\it{Misreporting and verification.}}}

The fund manager has an incentive to make a false report about the fund proceeds. Namely, by reporting $\tilde{y}$, she pays out $(1-\phi)\tilde{y}$ to investors and captures $y-(1-\phi)\tilde{y}$ on her own, suggesting that reporting $\tilde{y}=0$ would be optimal.\footnote{Technically, the manager earns an infinite payment by reporting $\tilde{y}=-\infty$, while we assume that investors must receive non-negative payoffs from the fund (i.e., limited liability), thereby imposing $\tilde{y}\ge0$.} To counteract this intentional misreporting, we assume that a investor pays cost $C$ when she becomes a fund investor. This cost gives her the ability to verify the manager's investment strategy $x$ \textit{ex-post} in the end of $t=1$. Together with the market data $(\delta,p)$, she can verify $y=(\delta-p)x+M$, thereby disproving $\tilde{y}\neq y$.\footnote{We implicitly assume that the manager is penalized and incurs a negative large utility if her intentional misreporting is detected.} Otherwise, the manager can intentionally misreport $\tilde{y}=0$, as there are levels of noise in the fund information ($e$) and noise trading ($u$) that are consistent with the report, i.e., she claims that the investment was $\tilde{x}\equiv-M/(\delta-p)$, while realized noise terms were $\tilde{e}=f-\tilde{x}$ and $\tilde{u}=(x+u)-\tilde{x}$, making $f$ and $p$ consistent with $\tilde{y}=0$. We assume that the verification cost is increasing in the degree of fund opacity, $C'(\omega_{e})>0$, while costless verification is possible only if the fund is transparent, $C(0)=0$. This follows from the intuition that verification requires more intensive analyses of fund information $f$ when the fund is more opaque. 

\paragraph{\normalfont{\it{Sunspot shock.}}}
To formalize the optimization problems in $t=0$ in the rational expectations framework, we introduce an equilibrium-selection device in cases of multiple equilibria. In particular, following the most common approach, we assume that market participants condition their actions on the realization of an exogenous \textit{sunspot} shock, characterized by the binary random variable $z\in\{0,1\}$.\footnote{The sunspot equilibrium is proposed by \citet{diamond1983bank} as an equilibrium selection device in the context of bunk runs and formalized in the equilibrium analyses by the subsequent studies, such as \citet{cooper1998bank}.} Namely, whenever parameters admit multiple equilibria, market participants play the high-$\beta$ equilibrium if a spot appears on the sun ($z=1$), whereas the low-$\beta$ equilibrium is realized if no spots are observed ($z=0$). Accordingly, the high-$\beta$ equilibrium is realized with probability $\rho_{H}\equiv\Pr(z=1)$, while the complementary probability is denoted as $\rho_{L} = 1-\rho_{H}$. We assume that $z$ is realized at the beginning of the trading stage in $t=1$. In what follows, the trading intensity in the high-$\beta$ and low-$\beta$ equilibria are denoted as $\beta = \beta_{H}$ and $\beta_{L}$, respectively. Also,  for consistency, we define $\rho_{j} = 1$ for $j=H,L$ when the equilibrium with $\beta = \beta_{j}$ is unique.

\paragraph{\normalfont{\it{Optimization.}}}
For each level of $\beta$, equation (\ref{eq:mi_max}) leads to the following expected payoff to individual investors:

\begin{equation}
     V_{I}(\beta) \equiv \mathrm{E}\left[\frac{1-\phi}{n}\mathrm{E}[\pi|f] -\phi\frac{\theta}{n}\mathrm{E}[\pi|f] \right] = \frac{1-\phi}{n^2}\frac{\beta \omega_{s}\omega_{u}}{\beta^2 \omega_u + \omega_s},\label{eq:pibeta} \\
     %&=\mathrm{E}\left[\frac{1-\phi}{n^2}\mathrm{E}[\pi|f]\right] \\
     %&\nonumber
\end{equation}
where $\beta = \beta_{H}$ or $\beta_{L}$. The first term in the middle of (\ref{eq:pibeta}) represents the share of the trading profit allocated to each investor, and the second term is the cost assosiated with delegation, $\phi m$, in the equilibrium. The last equality arises from the expected trading profit, $\mathrm{E}[\pi] = \frac{\beta \omega_{s}\omega_{u}}{\beta^2 \omega_u + \omega_s}$. Therefore, an investor is willing to participate in the fund and pays the verification cost if, and only if, 
\begin{equation}
