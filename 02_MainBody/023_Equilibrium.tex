...
In determining the capital allocation in \eqref{eq:mi}, each investor seeks to compute the expected fund profits $\mathrm{E}[\pi|f]$. This process, in turn, requires the investor to back out the realized skill level of the manager $|s|$ from fund information $f=x+\eta$. 
...
\subsubsection{Fund Opacity and Multiple Equilibria}
...
Having compared the two stable equilibria at a fixed level of fund opacity, we now examine how fund opacity $\omega_{\eta}$ itself shapes equilibrium trading behavior, fund performance, and investor welfare.
