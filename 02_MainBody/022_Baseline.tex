...
%===========================================
%This section presents a one-period trading model à la \citet{kyle1985continuous} with delegated asset management. In this baseline model, fund opacity is exogenously given. In Section \ref{sec:endogenousopacity}, a fund manager endogenously chooses opacity.
...
To prevent this intentional misreporting, we assume that each investor pays cost $C(\omega_e, M)$ if she invests in the fund. This cost gives investors the ability to verify (i.e., back out) the manager's investment strategy $x$ from the fund information in the end of the investment period, when they also observes the market data on the asset's price and the payoff, $(\delta,p)$. Otherwise, the manager can intentionally misreport $\tilde{y}=0$, as there are levels of noise in the fund information ($e$) and noise trading ($u$) that are consistent with the report, i.e., the manager claims that the investment was $\tilde{x}\equiv-M/(\delta-p)$, while realized noise terms were $\tilde{e}=f-\tilde{x}$ and $\tilde{u}=(x+u)-\tilde{x}$, making $f$ and $p$ consistent with $\tilde{y}=0$. For tractability, we assume that the verification cost for each investor takes the form of $C(\omega_e, M) = \frac{c}{M}\omega_e$. Namely, the verification cost is increasing in the degree of fund opacity, while costless verification is possible if the fund is perfectly transparent, $C(0, M)=0$. This assumption follows from the intuition that verification requires more intensive analyses of fund information $f$ when the fund is more opaque. Moreover, the cost function implies that the total verification cost is shared among fund investors of measure $M$. This is realistic, as the verification in reality often involves collective auditing and, once information about $x$ is uncovered, it is a common good for all investors.\footnote{The qualitative results ramins unchanged if we assume that a part of the verification cost cannot be shared by investors.}
...
\begin{equation}
    \max_{m_i\in\{0,1\}}\mathrm{E}\left[\frac{m_i}{M}(1-\phi)y -m_{i} - C(\omega_e,M)m_{i}|f\right].\label{eq:investorproblem}
\end{equation}
...
\begin{equation}
    \max_{m_i\in\{0,1\}} \frac{m_i}{M}(1-\phi)\mathrm{E}\left[\pi|f\right]-\left(\phi + C(\omega_e,M)\right) m_i.\label{eq:mi_max}
\end{equation}
