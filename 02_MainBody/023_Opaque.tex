...
More surprisingly, our model suggests that a low fund management fee ($\phi$) tends to generate equilibrium multiplicity though a hightened flow-performance sensitivity. Intuitively, each investor's fund delegation becomes more sensitive to the expected fund return when she captures a large portion of it (i.e., $\phi$ is low), while the fee rate does not directly affect the manager's decision making. This result may sound counter intuitive, as a high performance-based fee in the fund management industry is often critisized as the source of excessive risk taking. XXXX
