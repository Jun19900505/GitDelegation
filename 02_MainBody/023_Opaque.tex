...
\begin{proposition}[Equilibrium characterization]\label{prop:eqm}
There exists a critical value of the fund opacity, $\tilde{\omega}_{e}=8\omega_{u}$, and two thresholds for the flow-performance sensitivity, $\theta_{L}$ and $\theta_{H}$ $(\geq\theta_{L})$, such that, $\theta\geq \theta_{H}$ leads to a unique high-$\beta$ equilibrium, while $\theta\leq\theta_{L}$ leads to a unique low-$\beta$ equilibrium. Furthermore,
\begin{enumerate}[topsep=2.5pt, label = (\roman*)]
 \item if $\omega_{e}\leq \tilde{\omega}_{e}$ then $\theta_{H}=\theta_{L}$, hence the equilibrium is unique; and
 \item if $\omega_{e}> \tilde{\omega}_{e}$ then $\theta_{H}>\theta_{L}$, and $\theta\in(\theta_{L},\theta_{H})$ leads to multiple equilibria.
\end{enumerate}
\end{proposition}
