...
\begin{lemma}\label{lem:reputation}
	...
	\begin{equation}
     ...
     \begin{equation}
         \hat{\omega}_{s} = \mathrm{Var}[s|f] = \frac{\omega_{s}\omega_{e}}{\beta^2 \omega_{s} + \omega_{e}}.\label{eq:cond_var}
     \end{equation}
 \end{lemma}
...
Our model suggests that fragility may occur when funds exhibit an intermediate degree of opacity ($\omega_e$) and flow–performance sensitivity ($\theta$) is sufficiently high. First, 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
...
\caption*{\footnotesize{Note: The figure plots the manager's expected fee income at the low-$\beta$ equilibrium (solid line) and high-$\beta$ equilibrium (dashed line) for different levels of skill, $|s|$. The parameter values used in the figures are $\omega_{s}=1$, $\omega_{u}=2$, $\omega_{e}=38$, and $\theta=27$.}}\label{fig:wageSens}
\end{figure}
