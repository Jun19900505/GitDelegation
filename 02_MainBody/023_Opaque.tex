...
\begin{lemma}\label{lem:reputation}
	...
	\begin{equation}
     ...
     \end{equation}
 \end{lemma}
 Equation (\ref{eq:hats}) describes how investors update their belief about the manager's skill using $f=x+e$, where $\hat{s}$ is referred to as the \textit{reputation} in the following discussion. According to equation (\ref{eq:hats}), $\xi$ measures how much clients rely on the opaque fund information, determining the sensitivity of the reputation to the fund's trading strategy. Holding $\beta$ constant, $\xi$ is large when investors' prior is unreliable ($\omega_{s}$ is large) and the fund information is transparent ($\omega_{e}$ is small). More importantly, $\xi$ itself also depends on the trading strategy, $\beta$, as a more intensive trading strategy makes the fund information more informative about $s$.
...
\end{equation}
where we omit the terms unrelated to $x$. As in the fully transparent benchmark in Section \ref{sec:Model}, the first two terms of equation (\ref{eq:managerproblem opaque}) represent the expected trading profit, and the third term corresponds to her fee income from the fund flow, $M$. Similar to the benchmark case, the fund flow increases with the manager's risky investment, $|x|$, because she can potentially inflate her reputation $|\hat{s}|$ by placing a large order (either buy or sell). Contrary to the transparent case, however, the reaction of the fund flow to the trading strategy is now influenced not only by the flow-performance sensitivity, $\theta$, but also by the clients' updating coefficient, $\xi$ in (\ref{eq:xi}). This term kicks in because clients cannot perfectly observe the manager's trading strategy when the fund is opaque. Importantly, clients' estimate of the manager's skill and, therefore, the reaction of the fund flow to the risky investment depend on the risky trading strategy itself: the more intensive the trdaing strategy, the more reliable the fund information in estimating the manager's skill. This structure is captured by the dependence of $\xi$ on $\beta$ in equation (\ref{eq:xi}) and leads to the strategic complementarity between the manager's risk taking and her reputation held by clients.
...
Proposition \ref{prop:eqm} and Figure \ref{fig:theta} indicate that, for a given value of fund opacity $\omega_{e}$, a weak flow-performance sensitivity supports only the low-$\beta$ equilibrium, while increases in $\theta$ can generate the high-$\beta$ equilibrium, potentially leading to multiple equilibria.\footnote{Lemma \ref{lem:theta_s} in Online Appendix \ref{proofendogenous} analytically characterizes the upward-sloping $\theta_{H}$ and $\theta_{L}$ in relation to $\omega_{e}$, as presented in Figure \ref{fig:theta}.} This result is intuitive, as $\theta$, together with $\xi$, determines the sensitivity of the fund flow to the trading strategy, thereby influencing the degree of strategic compelementarity. 
When $\theta$ is small, the fund flow's reaction to changes in the expected investment profit becomes weak, and the manager’s showing-off motive is less significant. Consequently, the hiding motive prevails, prompting her to trade less aggressively and leading to the low-$\beta$ equilibrium. Conversely, when $\theta$ is large, the fund flow becomes highly sensitive to the fund information, thereby encouraging the manager to show off her skill by trading intensively. This scenario gives birth to the high-$\beta$ equilibrium.

Overall, the type of equilibrium depends on the combination of $\theta$ and $\omega_{e}$, as illustrated in Figure \ref{fig:theta}. The tradeoff between the hiding and the showing-off motive tends to balance and generates multiple equilibria when the fund is relatively opaque and the fund flow is moderately sensitive to the expected investment performance. Otherwise, one of the conflicting motives dominates the other, making either one of the Kyle-like and flow-driven equilibria a unique equilibrium.
...
In the high-$\beta$ equilibrium, the manager reveals more information about $s$, thereby improving the precision of investors' estimation, as statement \ref{item:5} demonstrates. This makes the fund flows more responsive to skill, as investors can assess it more accurately.
...
The fund opacity also has an implication for the fund performance. For each level of $\beta$, the unconditional expected trading profit of the fund becomes $\mathrm{E}[\pi] = \frac{\beta\omega_{u}\omega_{s}}{\beta^2 \omega_{u} + \omega_{s}}$ and is decreasing in the trading intensity, $\beta$. This is due to the showing-off motive: the manager trades too aggressively compared to the profit-maximizing level of $\beta$. It revelas too much private information to the market maker and reduces the trading profit, while the manager is compensated for this loss by attracting the fund flow. The opacity, conversely, diminishes the showing-off motive and induces the manager to take more profit-driven strategies (Corollary \ref{cor:beta opacity}), leading to the following result. 
\begin{corollary}
    The expected fund performance is monotonically increasing in the fund opacity both in the high- and low-$\beta$ equilibria.
\end{corollary}
Figure XXX plots the expected profit, $\mathrm{E}[\pi]$, in relation to the fund opacity, $\omega_{e}$, which traces the reactions of $\beta$ in Figure \ref{fig:opacitybeta}. Once again, multiple equilibria support somewhat non-trivial relationship between the fund opacity and its expecter performance observed in the real market. With intermediate levels of fund-client informational transparency, the market's belief can coordinate on substantially different equilibria, leading similar funds to exhibit divergent performance outcomes. Our model implies that empirical studies could find weak or even contradictory correlations between fund transparency and returns, as the prevailing equilibrium is an outcome of market sentiment.
