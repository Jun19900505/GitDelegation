...
Panel (a) of Figure \ref{fig:exog} shows that if $\omega_{e}$ is large, there is a unique ``low-$\beta$ equilibrium,'' in which  $\beta$ is close to the level without delegated asset management. Panel (b) shows that a large $\omega_{e}$ leads to a unique ``high-$\beta$ equilibrium,'' in which $\beta$ is much larger than that in the original \citet{kyle1985continuous} model.
...
There exists a critical value of the fund opacity, $\tilde{\omega}_{e}$, and two thresholds of the flow-performance sensitivity, $\theta_{L}$ and $\theta_{H}$ ($\ge \theta_{L}$), such that, $\theta\geq\theta_H$ leads to a unique high-$\beta$ equilibrium, while $\theta\leq\theta_L$ leads to a unique low-$\beta$ equilibrium. Furtheremore,
...
When $\theta$ is small, the reaction fund flows to changes in the expected investment profit becomes weak, and the manager’s showing-off motive is less significant. Consequently, the hiding motive prevails, prompting her to trade less aggressively and leading to the low-$\beta$ equilibrium. Conversely, when $\theta$ is large, fund flows become highly sensitive to the fund information, thereby encouraging the manager to show off her skill by trading intensively. This scenario gives birth to the high-$\beta$ equilibrium.
...
\item \label{item:2} the manager's expected performance given the realized skill, $\mathrm{E}[(\delta-p)x|s]=\frac{\beta\omega_{u}s^{2}}{\beta^{2} \omega_{s}+\omega_{u}}$, is lower;
