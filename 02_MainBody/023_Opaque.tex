...
\item \label{item:5} Clients' estimate of the manager's skill is more precise, that is, $\hat{\omega}_{s}=\mathrm{Var}[s|f] = \frac{\omega_{s}\omega_{u}\omega_{\delta}}{\omega_{s}(\beta^{2}\omega_{\delta}+\omega_{u})+\omega_{u}\omega_{\delta}}$ is lower.
\end{enumerate}
\end{corollary}
...
In the high-$\beta$ equilibrium, the manager reveals more information about $s$, thereby improving the precision of clients' estimation, as statement \ref{item:5} demonstrates. This makes fund flows more responsive to skill, as clients can assess it more accurately.
